...
test.afterAll('Remove any synthetic recipe, labels, and group that remain', async ({ request }) => {
  if (recipeUrl) await request.delete(`/api/v1/recipes/${recipeUrl}`, { headers })
  for (const id of labelIds) await request.delete(`/api/v1/recipe-labels/${id}`, { headers })
  if (groupId) await request.delete(`/api/v1/recipe-label-groups/${groupId}`, { headers })
})
...
test('Recipes can be created, labeled, read, and deleted, and their invariants hold', async ({ request }) => {
  ...
  await test.step('View the newly created recipe with its parsed content', async () => {
    const res = await request.get(`/api/v1/recipes/${slug}`, { headers })
    expect(res.status()).toBe(200)
  ...
  await test.step('At most one label from a group can apply to a recipe', async () => {
    const groupRes = await request.post('/api/v1/recipe-label-groups', {
      headers, data: { name: `checkly-ethnicity-${Date.now()}` },
    })
    expect(groupRes.status()).toBe(200)
    groupId = (await groupRes.json()).id

    const indianRes  = await request.post('/api/v1/recipe-labels', { headers, data: { name: `indian-${Date.now()}`,  'group-id': groupId } })
    const mexicanRes = await request.post('/api/v1/recipe-labels', { headers, data: { name: `mexican-${Date.now()}`, 'group-id': groupId } })
    ...
    await test.step('Two labels from the same group are rejected with a 400', async () => {
      const res = await request.put(`/api/v1/recipes/${slug}`, { headers, data: { 'label-ids': [indianId, mexicanId] } })
      expect(res.status()).toBe(400)
    })

    await test.step('A single label from the group is accepted', async () => {
      const res = await request.put(`/api/v1/recipes/${slug}`, { headers, data: { 'label-ids': [indianId] } })
      expect(res.status()).toBe(200)
    })
  })
  ...
  await test.step('Scraping a disallowed URL is rejected without fetching it', async () => {
    ...
    const res = await request.post('/api/v1/recipes/scrape', {
  ...
  await test.step('Delete the recipe', async () => {
    const res = await request.delete(`/api/v1/recipes/${slug}`, { headers })
    expect(res.status()).toBe(200)
    recipeUrl = null
  })
...
test('Recipes are readable publicly but only writable by authenticated users', async ({ request }) => {
  ...
  await test.step('Anyone can list recipe labels', async () => {
    const res = await request.get('/api/v1/recipe-labels')
    expect(res.status()).toBe(200)
  })

  await test.step('Creating a recipe without a token is rejected with a 401', async () => {
    const res = await request.post('/api/v1/recipes', { data: { content: { title: 'unauthorized', ingredients: [] } } })
    expect(res.status()).toBe(401)
  })

  await test.step('Scraping without a token is rejected with a 401', async () => {
    const res = await request.post('/api/v1/recipes/scrape', { data: { url: 'http://example.com/recipe' } })
    expect(res.status()).toBe(401)
  })
})
