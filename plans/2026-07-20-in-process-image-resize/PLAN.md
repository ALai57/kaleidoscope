...
**Architecture:** On upload the app writes the raw to the store, then — in **one DB transaction** — inserts the photo row, the version rows, and one `media_resize_jobs` row (a Transactional Outbox: no window where the raw exists with no job, or a job with no raw). A worker loop claims jobs with `SELECT … FOR UPDATE SKIP LOCKED` and runs up to `:max-concurrent` at a time (**default 1** = ⌊512 MB / one 1920×1080 decode⌋), reading the raw from the media store, resizing with **Thumbnailator (subsampled decode)**, and `put-file`-ing each rendition into the same store the raw lives in — so renditions land in `kal-media-prod` (prod) or `kal-eph-<slug>-media` (ephemeral) automatically. The worker's *logic* is decomplected from *where it runs* (a background thread in the web process today; the seam to promote it to a separate Fly process later is preserved but **not built here**). Jobs are durable, idempotent (re-run overwrites the same keys), retried with linear backoff, and reaped on lease expiry by an independent loop; a resize unit has a hard timeout so a hanging decode can't freeze a slot; failures are logged + traced + counted.
...
- `run-worker! [{:keys [database media-store owner poll-ms lease-seconds max-concurrent] :as ctx} running?]` → `{:stop! fn}`. Starts **two** single-purpose loops: a **drain** loop (`drain-once!`, sleep `poll-ms` when idle) and an **independent reaper** loop (`reap-stalled-jobs!` every `lease-seconds/2`), so a blocked/hung drain can't starve recovery. Clean shutdown via `running?`.
...
**Interfaces:** `new-image` becomes: compute `raw-key`; **`put-file` the raw to the store FIRST** (fail → honest error, nothing committed); then **one `with-transaction`**: `create-photo!`, `create-photo-version-2!`, and `media-resize-jobs/enqueue-job! {:photo-id :hostname :raw-path raw-key}`. The `notify-image-resizer!` component, the `write-location`/`s3://…` message block, and the `:message-attributes` are all **removed** — the worker reads the store directly, so there is no URL contract. (Grep for `write-location`; if `new-image` was its only consumer, delete it + `photo_resize_contract_test`.)
...
- `in-process` (default) → `resize-enqueue!` inserts a job **and** the web process starts **one** `resize/run-worker!` on boot (guarded so exactly one starts; shutdown hook stops it). `:max-concurrent` from `KALEIDOSCOPE_RESIZE_MAX_CONCURRENT` (default `"1"`).
...
- [ ] **Step 3:** `docs/operations.md` — document: the `media_resize_jobs` queue + the Transactional Outbox ordering; the bounded worker (`:max-concurrent` arithmetic on 512 MB) + subsampled decode + `MAX-SOURCE-PIXELS` guard; the linear backoff + `max_attempts` + independent reaper + lease default (state it, comfortably > `RESIZE-TIMEOUT-MS`); the **monitoring surface** (ERROR log + OTEL span event on terminal fail + `failed-count` — because the worker is outside Bugsnag's request scope); the in-web-thread topology and the documented (unbuilt) promote-to-separate-process path; and that the AWS SNS/SQS/Lambda resizer (`../image-resizer`) is retired (tear down its Terraform once prod resize is verified).
...
1. **Queue over bounded-inline — with the alternative kept live.** The review split: a `Semaphore(1)` inline executor (van Rossum) also bounds heap and decouples the request, with no table/reaper/backoff/dual-engine split — and renditions are *recomputable*, so "durability" guards a non-catastrophe. We keep the **queue** because (a) it is the idiom the sibling `workflow-postgres-job-queue` establishes, so a second kind of background work adopting it is the consistent, "one obvious way" choice, and one worker can later drain both; (b) the Transactional Outbox (Decision 4) makes upload→resize integrity a real DB property, which the fire-and-forget executor can't offer. **Reversal is cheap and documented:** if the workflow queue never lands, collapse this to `Semaphore(1)` inline — delete the table/reaper/backoff, keep the pure `resize-renditions` core + the outbox ordering untouched. The pure core is what makes either topology a small change.
2. **In-web worker only; separate process is a future flip, not built now.** Building the `worker` `-main` branch + `WORKER_INLINE` matrix now = maintenance and test surface for a path that, on a 1-machine deploy, never runs (and doubles machine cost if it does). YAGNI: keep the worker a plain fn (the seam), run it as an in-web thread, and add the separate Fly process the afternoon a second machine actually exists.
...
- **Spec coverage:** in-process resize replacing the Lambda (Tasks 3–5) ✅; Postgres queue mirroring the sibling incl. the nullable-UNIQUE idempotency (Tasks 1–2, Decision 3) ✅; Transactional Outbox ordering (Task 5, Decision 4) ✅; bounded concurrency as a value + subsampled decode + pixel guard (Tasks 3–4, Decisions 6, Constraints) ✅; timeout + independent reaper + crash-vs-poison attempts (Task 4, Decision 7) ✅; monitoring surface (Task 4/7, Decision 8) ✅; pure backoff/time (Task 3, Decision 5) ✅; in-web-only worker, separate process cut (Task 6, Decision 2) ✅; config/docs/AWS retirement + acceptance gate (Task 7) ✅; queue↔Semaphore(1) reversal documented (Decision 1) ✅.
