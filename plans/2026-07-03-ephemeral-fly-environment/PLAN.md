...
5. `fly apps create kal-eph-spike`; `fly secrets set` with the branch's connection string (`neonctl connection-string eph-spike`) plus the existing Auth0, Bugsnag, `AWS_ACCESS_KEY_ID`, and `AWS_SECRET_ACCESS_KEY` secrets. (No `ANTHROPIC_API_KEY` — prod itself runs the AI features mocked today, so this spike matches that rather than introducing a new real dependency.) **AWS credentials are required even for this backend-only spike** — see the note below on why `/` isn't actually host-scoped.
6. `task build:uberjar` — the Dockerfile only `COPY`s a pre-built `target/kaleidoscope.jar`; it does not compile Clojure. `fly deploy` will silently redeploy stale code if this step is skipped.
7. `fly deploy -c fly.eph-spike.toml` — deploys the freshly built jar (the `role-domain` authorization change from decision #6 is included, but is a no-op for every real hostname).

**Correction found while running this spike**: `/` was assumed to 404 for an unrecognized host, matching the "no static-content code changes exist yet" framing below. That's wrong — `kaleidoscope.clj:121-123` hardcodes `/` (and its `static/*` assets, the SPA shell bundle) to always resolve to the shared `kaleidoscope.client` bucket **regardless of the request's Host**, independent of any per-host adapter map. Without AWS credentials, this fails with `"Unable to fetch credentials"` and crashes as an unhandled 500 (an existing gap in how that S3 anomaly is handled — not something fixed here). With AWS credentials present, `/` correctly returns 200 with the real SPA shell HTML, exactly like prod. So AWS credentials aren't a Phase 4/5-only concern — they're needed from Phase 2 onward for `/` to behave sanely at all.
...
- **Bugsnag**: trigger a deliberate server error (e.g. hit a route that throws) and confirm it appears in the Bugsnag dashboard. At the time this spike ran, it showed up tagged `production` regardless of origin — `BugsnagClient` never set `releaseStage` — but this at least confirmed the API key and wiring worked end-to-end. **Since fixed** (see the `KALEIDOSCOPE_ENV` note below): set `KALEIDOSCOPE_ENV=kal-eph-<slug>` (or similar) as a Fly secret on each ephemeral app and Bugsnag reports, OTEL traces, and JSON logs will all agree on that environment name instead of defaulting to `production`.
...
- `/` returns 200 with the real SPA shell HTML (see correction above — it's not host-scoped, so it works today given AWS credentials, with no static-content code changes needed).
...
- `scripts/ephemeral/deploy-app` — runs `task build:uberjar` first (the Dockerfile only `COPY`s a pre-built `target/kaleidoscope.jar`; it never compiles Clojure, so skipping this step silently redeploys stale code). Then generates a per-run `fly.toml` from the root one as a template: app name, `OTEL_SERVICE_NAME`, `KALEIDOSCOPE_ENV` (set to `kal-eph-<slug>`, tags Bugsnag `releaseStage`, the OTEL `deployment.environment` resource attribute, and JSON log lines — see the Bugsnag note in Phase 1), the Neon branch's connection string as secrets, and scale-to-zero VM settings (`auto_stop_machines = true`, `auto_start_machines = true`, `min_machines_running = 0`, same `1gb` shared-cpu-1x VM — the Dockerfile hardcodes `-Xmx512m`, so shrinking further risks OOM). Then `fly apps create`, `fly secrets set` for the Neon connection string, Bugsnag key, **and `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`** (needed from this phase on — see the correction in Phase 1: `/` always hits the shared `kaleidoscope.client` bucket regardless of host, so it 500s without real AWS credentials even though no other static content is host-scoped yet), then `fly deploy -c <generated-toml>`.
- `scripts/ephemeral/smoke-test` — `curl /ping`, `curl /` (expect 200 — see Phase 1 correction, this isn't host-scoped so it works today), plus an Auth0 M2M token + one authenticated write to validate DB+Auth0+authz together.
...
| **AWS** | **Phase 2**: reuse prod's existing `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` Fly secrets as-is — every ephemeral app needs read access to the shared `kaleidoscope.client` bucket for `/` to resolve (see Phase 1's correction), regardless of whether any other static-content code has landed yet. **Phase 4**: additionally create one persistent `kal-ephemeral` S3 bucket, once (console or `aws s3 mb s3://kal-ephemeral`), and scope its own credentials to `s3:PutObject`/`GetObject`/`DeleteObject` on that bucket only — no `CreateBucket`/`DeleteBucket` needed since environments isolate via key prefix, not bucket lifecycle | Phase 2 (reuse), Phase 4 (new bucket) | The Phase 4 bucket setup is a one-time, out-of-band prerequisite — not scripted, same shape as the Neon `staging` branch setup. All credentials stored in a new untracked `.env.fly.staging` file (same shape as existing `.env.fly.prod`, already covered by the `*.env*` gitignore rule). |
...
| **Bugsnag** | Reuse the existing API key; set `KALEIDOSCOPE_ENV=kal-eph-<slug>` as a Fly secret/env on each ephemeral app | Phase 1 (fixed), Phase 2/5 (wired into `deploy-app`) | Previously a gap: `BugsnagClient` never set `releaseStage`, so ephemeral-env errors showed up tagged `production`, indistinguishable from real prod errors. **Fixed**: `KALEIDOSCOPE_ENV` now drives Bugsnag `releaseStage`, the OTEL `deployment.environment` resource attribute, and JSON log lines, defaulting to `production` when unset (`init/env.clj:environment-name`). |
| **OTEL / Grafana Tempo / Sumo Logic** | No configuration needed beyond `OTEL_SERVICE_NAME` and `KALEIDOSCOPE_ENV` | Phase 1 | `kaleidoscope-otel-collector` is reached via Fly's `.internal` 6PN network automatically for any app in the same org — zero extra config. Traces are separated from prod both by the per-environment `OTEL_SERVICE_NAME` and by the `deployment.environment` resource attribute. **Sumo log shipping is explicitly out of scope** (the log shipper filters by `APP_NAME=kaleidoscope-publishing`) — use `fly logs -a <ephemeral-app>` directly instead. |
| **`.env.fly.staging`** (new, local, untracked) | `NEON_API_KEY` + `NEON_PROJECT_ID` (used by `neonctl` to create/delete branches and fetch each branch's connection string at `up`/`down` time), plus AWS credentials | Phase 2 (Neon + reused AWS creds), Phase 4 (new `kal-ephemeral`-scoped AWS creds) | One-time manual setup by the developer. No static `KALEIDOSCOPE_DB_HOST/_NAME/_USER/_PASSWORD` lives here — each `env:up` fetches a fresh connection string for that run's branch via `neonctl connection-string eph-<slug>`. |
