...
**Goal**: de-risk the core assumption — that an unmodified deploy of this app, pointed at a brand-new Neon branch and a brand-new Fly app, actually works end-to-end against every real dependency (Auth0, Anthropic, Bugsnag, OTEL/Tempo, Sumo Logic) — before investing in any automation.

**Do by hand**:
1. One-time: `neonctl branches create --project-id <proj> --name staging --parent main` (see Third-party configuration below).
2. `neonctl branches create --project-id <proj> --name eph-spike --parent staging`.
3. Copy `fly.toml` → a scratch `fly.eph-spike.toml`, changing only the app name (`kal-eph-spike`) and `OTEL_SERVICE_NAME`.
4. `fly apps create kal-eph-spike`; `fly secrets set` with the branch's connection string (`neonctl connection-string eph-spike`) plus the existing `ANTHROPIC_API_KEY`, Auth0, and Bugsnag secrets.
5. `fly deploy -c fly.eph-spike.toml` — deploys the exact code that would be built from the current branch today. No code changes.
...
| **Anthropic** | Reuse the existing `ANTHROPIC_API_KEY` | Phase 1 | Set as a Fly secret on each ephemeral app. See cost note below. |
