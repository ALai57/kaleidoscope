...
(deftest create-and-retrieve-album-test
  (let [database (embedded-h2/fresh-db!)]
    (testing "3 example-albums seeded in DB"
      ...
      (is (= 3 (count (albums-api/get-all-albums database)))))
    ...
    (let [{:keys [id]} (albums-api/create-album! database example-album)]
      ...
      (testing "Can retrieve example-album from the DB"
        (is (match? example-album (albums-api/get-album database (:album-name example-album))))
        (is (match? example-album (albums-api/get-album-by-id database id))))
    ...
    (let [album-id  (:id (first (albums-api/get-all-albums database)))
      ...
      (testing "Content gets added to the album"
        (let [album-content (albums-api/add-photos-to-album! database album-id photo-ids)]
          ...
          (is (= 3 (count (albums-api/get-album-contents database album-id))))

          (testing "Content is deleted"
            (albums-api/remove-content-from-album! database album-id (map :id album-content))
            (is (= 0 (count (albums-api/get-album-contents database album-id))))))))))

(deftest get-all-album-contents-test
  (let [database (embedded-h2/fresh-db!)]
    ;; 3 example-albums seeded in DB
    (let [album-ids (map :id (albums-api/get-all-albums database))
          photo-ids (map :id (photos-api/get-all-photos database))]

      (testing "No contents in the album to start"
        (is (= [] (albums-api/get-all-contents database))))
      ...
      (testing "Contents come from different albums"
        (is (= 3 (count (set (map :album-name (albums-api/get-all-contents database)))))))
      )))
